...
class views_empresa:
    ...
    def list_empresas(request):
        try:
            ...
            return JsonResponse(
                {"data": empresas_lista, "success": True}, status=200
            )
        except Empresa.DoesNotExist:
            return JsonResponse(
                {"error": "Empresas não encontradas"}, status=404
            )
    ...
    def get_empresa(request, id = None):
        try:
            ...
            return JsonResponse(
                {"data": empresa_json, "success": True}, status=200
            )
        except Empresa.DoesNotExist:
            return JsonResponse(
                {"error": "Empresa não encontrada"}, status=404
            )
        except Exception as e:
            return JsonResponse({"error": str(e)}, status=500)
    ...
    def update_empresa(request, id):
        if request.method == "PUT":
            ...
            try:
                ...
                empresa.nome_responsavel = data.get("nome_responsavel", empresa.nome_responsavel)
