...
class RegistroDiarioPDV(models.Model):
    id_registro_diario = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    ...
    saldo_inicial = models.DecimalField(max_digits=10, decimal_places=2,editable=False, default=0, null=True)
    saldo_final = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True)
    horario_inicial = models.CharField(editable=False, max_length=50)
    ...
    class Meta:
        unique_together = ('pdv', 'dia')  # Garantir que não haja duplicatas de pdv e dia
...
class TransacaoPDV(models.Model):
    ...
    registro_diario = models.ForeignKey(RegistroDiarioPDV, on_delete=models.CASCADE, blank=True)
...
class HistoricoAlteracoes(models.Model):
    id_historico = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    entidade = models.CharField(max_length=100)  # Nome da entidade alterada, ex: 'PDV', 'AssociadoPDV'
    entidade_id = models.UUIDField()  # ID da entidade alterada
    tipo_alteracao = models.CharField(max_length=50)  # Tipo de alteração, ex: 'criação', 'atualização', 'exclusão'
    campo_alterado = models.CharField(max_length=100, null=True, blank=True)  # Nome do campo alterado
    ...
    usuario = models.ForeignKey(Usuario, on_delete=models.SET_NULL, null=True)  # Usuário que realizou a alteração
    data_alteracao = models.DateTimeField(default=Utils.obter_data_hora_atual, editable=False)
