...
class DatabaseWrapper(BaseDatabaseWrapper):
        ...
        while conn is None:
            try:
                conn = Database.connect(
                    connstr, unicode_results=unicode_results, timeout=timeout
                )
    ...
    def init_connection_state(self):
        ...
        if drv_name.startswith("LIBTDSODBC"):
            try:
                ...
                if ver < (0, 95):
                    raise ImproperlyConfigured("FreeTDS 0.95 or newer is required.")
            except:
    ...
    def check_constraints(self, table_names=None):
        self._execute_foreach(
            "ALTER TABLE %s WITH CHECK CHECK CONSTRAINT ALL", table_names
        )
